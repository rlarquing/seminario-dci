...
export function ProfesoresTab() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.asignaturaId) {
      toast.error('La asignatura es requerida')
      return
    }
    if (!formData.nombreIglesia.trim()) {
      toast.error('El nombre de la iglesia es requerido')
      return
    }
    if (!formData.nombrePastor.trim()) {
    ...
    try {
      ...
      if ('caches' in window) {
        ...
        await cache.delete('/api/profesores')
      }
      fetchData(showDeleted)
    } catch (error) {
      console.error('Error al guardar profesor:', error)
      toast.error('Error al guardar el profesor: ' + (error instanceof Error ? error.message : 'Error desconocido'))
    }
  }

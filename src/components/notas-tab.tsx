...
export function NotasTab() {
  ...
  const fetchData = async () => {
    try {
      // Limpiar cache del service worker antes de obtener datos frescos
      if ('caches' in window) {
        const cache = await caches.open('seminario-dci-v3')
  ...
  const handleSaveAllNotas = async () => {
    ...
    try {
      ...
      if (allOk) {
        ...
        toast.error('Algunas notas no se pudieron guardar')
      }
    } catch {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            {selectedAlumno ? (
              ...
              <div className="h-full flex items-center justify-center text-gray-500 border rounded-md p-10 min-h-[300px]">
